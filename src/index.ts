...
const initialize = (apiKey: string, target: Target, options?: Options): Result => {
  ...
  const startStream = () => {
    ...
    readStream(request, processData);
  }

  async function readStream(request, dataCallback) {

    try {

      const resp = await fetch(request); // TODO figure out why this hangs on FF (116.0.2) when an error HTTP code is returned
      logDebug("Got RESP " + resp.status)
      if (resp.ok) {
        const reader = resp.body.getReader()
        const decoder = new TextDecoder();
        while (true) {
          const {done, value} = await reader.read();
          const data = decoder.decode(value, {stream: true}); // TODO check value == undefined
          if (done) {
            logDebug("last chunk " + data)
            dataCallback(data);
            return
          }
          logDebug("next chunk " + data)
          dataCallback(data);
        }
      } else {
        logError("Failed to connect to SSE stream HttpCode=" + resp.status + " HttpStatus=" + resp.statusText);
      }
    } catch (err) {
      logError("SSE stream failed: " + err);
    }
  }
